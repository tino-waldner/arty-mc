...
def create_session():
    ...
    adapter = HTTPAdapter(max_retries=retry, pool_connections=CONCURRENCY_LIMIT, pool_maxsize=CONCURRENCY_LIMIT)
    session = Session()
...
async def upload(local_path: str, remote_path: str, remote_repo_url=None, auth=None, progress_callback=None):
    ...
    for file in files:
        ...
        tasks.append(upload_file_limited(file, remote_url, session, auth, progress_callback))
...
async def download(remote_path: str, local_path: str, remote_repo_url=None, auth=None, progress_callback=None):
    ...
    tasks = [download_file_limited(rf, lf, session, auth, progress_callback) for rf, lf in files]
    await asyncio.gather(*tasks)
